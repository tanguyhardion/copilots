...
def _fmt_date_range(date_from: str, date_to: str) -> str:
    """Format date range: 'Jan. 2021 – Present'."""
    f = _fmt_date_part(date_from)
    t = _fmt_date_part(date_to)
    if f and t:
        return f"{f} – {t}"
    return f or t or ""


def generate_pptx_cv(cv_json: Dict[str, Any], output_path: str, template_path: Optional[str] = None) -> str:
    """
    ...
    full_name = f"{first_name} {last_name}".strip() or "Candidate Name"
    ...
    if tp_shapes:
        ...
        if len(tf.paragraphs) > 3:
            p3 = tf.paragraphs[3]
            p3.text = f"Mobile: {phone}" if phone else ""
            if p3.runs:
        ...
        if len(tf.paragraphs) > 4:
            p4 = tf.paragraphs[4]
            p4.text = f"Email: {email}" if email else ""
            if p4.runs:
    ...
    if t71_shapes:
        ...
        for row_idx in range(max_rows):
            ...
            if row_idx < len(projects):
                ...
                date_label = _fmt_date_range(d_from, d_to)
                ...
                if client and title:
                    ...
                    header_line = title or client or "Project"
